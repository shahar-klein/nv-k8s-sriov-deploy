...
set +e
#blocking
kubectl delete -f $D/iperf1.yaml 2>/dev/null
set -e

kubectl -v=6 create -f $D/iperf1.yaml

kubectl wait pod --for=condition=Ready -l k8s-app=iperf1-test --timeout=30s || (echo "ERROR starting iperf-test pods" ; exit 1)
...
kubectl exec -it node3-pod -- iperf -i 1 -c $IP_NODE2
RV=$?
if [ $RV -ne 0 ] ; then
	echo "ERROR $TEST"
	exit 1
